...
export default function AdminProperties() {
  ...
  const fetchProperties = useCallback(async () => {
    ...
    try {
      ...
      const json = await apiFetch(`/admin/properties?${params}`);
  ...
  const updateStatus = async (propertyId: string, status: string) => {
    ...
    try {
      await apiFetch(`/admin/properties/${propertyId}/status`, {
        method: 'PATCH',
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {properties.map((p: any) => {
            // CORRECTION : c'est "images" pas "propertyImages"
            ...
            return (
              ...
              >
                ...
                <div className="p-4">
                  ...
                  {/* CORRECTION : c'est "host" pas "user" */}
                  <p className="text-xs text-gray-400 mt-0.5">
                    par {p.host?.firstName} {p.host?.lastName}
                  </p>
                  ...
                  <div className="flex gap-2 mt-3 pt-3 border-t border-gray-50">
                    {p.status === 'DRAFT' && (
                      <button
                        ...
                        className="flex-1 text-xs font-medium py-2 rounded-lg bg-green-50 text-green-700 hover:bg-green-100 transition disabled:opacity-50"
                      >
                        {isActionLoading ? <i className="fas fa-spinner fa-spin" /> : 'Publier'}
                      </button>
                    )}
                    {p.status === 'PUBLISHED' && (
                      <button
                        ...
                        className="flex-1 text-xs font-medium py-2 rounded-lg bg-gray-100 text-gray-700 hover:bg-gray-200 transition disabled:opacity-50"
                      >
                        {isActionLoading ? <i className="fas fa-spinner fa-spin" /> : 'Archiver'}
                      </button>
                    )}
                    {p.status === 'ARCHIVED' && (
                      <button
                        ...
                        className="flex-1 text-xs font-medium py-2 rounded-lg bg-green-50 text-green-700 hover:bg-green-100 transition disabled:opacity-50"
                      >
                        {isActionLoading ? <i className="fas fa-spinner fa-spin" /> : 'Republier'}
                      </button>
                    )}
                  </div>
                </div>
