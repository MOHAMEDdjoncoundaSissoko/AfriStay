...
export default function PropertyReviews({ propertyId }: PropertyReviewsProps) {
  ...
  async function checkCanReview() {
    ...
    if (!token) return;
    try {
      const res = await fetch(`${API_URL}/api/reviews/can-review/${propertyId}`, {
      ...
      <div className="flex items-center justify-between">
        ...
        {canReview && !showForm && (
          <button
            ...
            className="px-5 py-2.5 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary/90 transition"
          >
        ...
        <div className="bg-gray-50 rounded-xl p-6 space-y-4 border border-gray-200">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Détaillez votre séjour..."
              rows={4}
          ...
          <div className="flex gap-3">
            <button
              ...
              className="px-5 py-2.5 text-sm border border-gray-200 rounded-lg hover:bg-gray-100 transition"
            >
              Annuler
            </button>
            <button
              onClick={submitReview}
              disabled={formRating === 0 || !formComment.trim() || submitting}
              className="px-5 py-2.5 bg-primary text-white rounded-lg text-sm font-medium hover:bg-primary/90 transition disabled:opacity-50"
            >
              ...
              <div key={review.id} className="bg-white border border-gray-100 rounded-xl p-5">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {review.title && (
                      <h4 className="font-semibold text-gray-900 mt-2 text-sm">{review.title}</h4>
                    )}
                        ...
                        <p className="text-xs font-medium text-primary mb-1">
                          ...
                          {review.hostRepliedAt && (
                            <span className="text-gray-400 font-normal ml-2">
                              {new Date(review.hostRepliedAt).toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' })}
                            </span>
                          )}
                        </p>
